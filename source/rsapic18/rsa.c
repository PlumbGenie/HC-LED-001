...
// non-recursive version of shift left by Joseph Julicher
// original version from the mp library according to the copywrite at the top of the file.
// non-recursive version uses elements from the original recursive version.
DIGIT_T mpShiftLeft(DIGIT_T a[], const DIGIT_T *b, size_t shift, size_t ndigits)
{
    size_t nw,i,bits,y;
    DIGIT_T mask, wordcarry,carry, nextcarry;
    const DIGIT_T *c;
    // shift whole digits
    if(shift >= BITS_PER_DIGIT)
    {
        nw = shift / BITS_PER_DIGIT;// number of digits to shift
        i = ndigits;				// number of digits in the number
        while(i--)					// for each digit
        {
            if (i >= nw)			// if the number needs to be shifted..
                a[i] = b[i-nw];		// shift it up..
            else
                a[i] = 0;			// shift in zero's at the end...
        }
        bits = shift % BITS_PER_DIGIT;
        wordcarry = b[ndigits-nw] << bits;
        c = a;
    }
    else
    {
    ...
    for (i = 0; i < ndigits; i++)
    {
        nextcarry = (c[i] & mask) >> y;
        a[i] = c[i] << bits | carry;
        carry = nextcarry;
    }
    // return the final carry
    return wordcarry | carry;
}


// non-recursive version of shift right by Joseph Julicher
// original version from the mp library according to the copywrite at the top of the file.
// non-recursive version uses elements from the original recursive version.
